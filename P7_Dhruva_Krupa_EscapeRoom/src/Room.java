...
public abstract class Room implements CommandHandler {
    ...
    /**
     * A description of the room that is printed at the beginning of the game and also when the look
     * command is executed.
     */
    private String description;

    /**
     * The introduction printed at the beginning of the game that describes the scenario for this
     * room.
     */
            ...
            if (scan.hasNext()) {
                ...
                if (com.equals("use")) {
                    ...
                    if (stuff.size() == 0) {
                        ...
                        System.out.print(
                                "You attempt to combine the following items: "
                                        + stuff.get(0).getName());
    ...
    /**
     * Adds an item to the room. If the item implements the CommandHandler interface and the app has
     * been set, the item is also added to the list of CommandHandlers for the app.
     *
    ...
    /**
     * Removes the item from the room. If the app has been set, the item is also removed from the
    ...
    /**
     * Returns a shallow copy of the list of items in the room. A shallow copy just means that a new
     * list is returned that contains all the same items as the original. Changing the order of the
     * items in the copy or adding or removing items from the copy will not be reflected in the
     * original list of items for the room because the copy is a different list; however, the items
     * in the copy are direct references to the actual items in the room, so changing the properties
     * of an item contained in the copy will still change the properties of the item in the room -
     * they are the same objects. We do not give direct access to the array because that would mean
     * the list of items could be modified directly, bypassing the add(item) and remove(item)
     * methods that perform additional actions that need to be done whenever an item is added or
     * removed from the room. It is good practice to protect your class from being broken by misuse.
     *
    ...
    /**
     * Returns a shallow copy of the list of recipes in the room. See getItems() for more
     * information on what it means to be a shallow copy.
     *
    ...
    /**
     * Attempts to combine the given items. If any recipes in this room match the items, the items
     * will be combined by calling combineInRoom on the recipe, passing this room. If the items
     * don't match a recipe, the onCombineFailed method will be called.
     *
    ...
    /**
     * Executed whenever a combine attempt fails. This method can be overridden for custom behavior,
     * but the default is to just inform the user that nothing happens.
     *
